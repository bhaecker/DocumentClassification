...
def make_split(split):
    ...
    for counter, label in enumerate(labels):
        ...
        images = np.stack((images[:][:][:], images[:][:][:], images[:][:][:]), axis=3)
        images = images.astype('float16')
        np.save('Data/Tobacco_test/' + label + '.npy', images)

        images = [cv2.resize(imageio.imread(path), (244, 244)) for path in unseen_array_split]
        images = np.asarray(images)
        images = np.stack((images[:][:][:], images[:][:][:], images[:][:][:]), axis=3)
        images = images.astype('float16')
        np.save('Data/Tobacco_unseen/' + label + '.npy', images)

        #label the classes
        target = np.zeros(len(labels))
        target[counter] = 1
        np.save('Data/Tobacco_train/' + label + '_target.npy', target)
        np.save('Data/Tobacco_test/' + label + '_target.npy', target)
        np.save('Data/Tobacco_unseen/' + label + '_target.npy', target)
