...
def experiment_accumulated_earlystopping(model_base_str,epochs_retrain,retrain_size,mini_batch_size,list_methods):
    ...
    Xunseen_orig, yunseen_orig = fetch_data('trans_unseen')
...
def experiment_accumulated_earlystopping_modelreuse(model_base_str,epochs_retrain,retrain_size,mini_batch_size,list_methods):
    ...
    for method in list_methods:
        ...
        print('training data concatenated')
        #while number_samples <= np.shape(Xunseen_orig)[0]:
        model = load_model(model_base_str)
        while np.shape(Xwinner)[0] > 0:
            #print(method.__name__,'at '+str(100*(np.shape(Xtrain_new)[0] - np.shape(Xtrain)[0])/np.shape(Xunseen_orig)[0])+' %')
            #just for debugging
            #if index == 1:
            ...
            #model_base = load_model(model_base_str)
            print('model loaded')
            model = retrain_early_stopping(model,epochs_retrain,mini_batch_size,Xtrain_new, ytrain_new, Xtest, ytest)[0]

            accuracy = tester(Xtest,ytest, model)[0]
            df.at[index, 'fraction used'] = round(1-np.shape(Xloser)[0]/np.shape(Xunseen_orig)[0],2)#number_samples
            df.at[index, str(method.__name__)] = accuracy
            print(df)
            Xwinner, ywinner, Xloser, yloser = seperation(Xloser, yloser, model, retrain_size, method)
